  ...
  return (
    <ul className={`menu ${mobileActive && 'active'} ${dark && 'dark'}`}>
      {navLinks.map((link, index) => (
        <li key={index}>
          <Link className={`nav-link ${isActive(link.path)}`} to={link.path} title={link.label}>
            <link.icon />
          </Link>
        </li>
      ))}
      ...
      >
        <a className={`${dropDown && 'active'}`}>
          ...
          <span className="notifications-length">{notify.data.filter((item) => !item.isRead).length}</span>
        </a>
        {dropDown && (
